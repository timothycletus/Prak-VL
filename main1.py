...
def draw_text_within_bounds(draw, word, font, max_width, max_height, position):
    """Draw text within the grid bounds, scaling if needed or wrapping the text."""
    word_bbox = draw.textbbox((0, 0), word, font=font)
    word_width = word_bbox[2] - word_bbox[0]
    
    # Check if word fits in the cell width
    if word_width > max_width:
        # Reduce font size to make the word fit
        while word_width > max_width and font.size > 10:  # Prevent too much reduction
            font = ImageFont.truetype(font.path, font.size - 1)
            word_bbox = draw.textbbox((0, 0), word, font=font)
            word_width = word_bbox[2] - word_bbox[0]

    # Draw the text after adjusting size
    word_height = word_bbox[3] - word_bbox[1]
    if word_height <= max_height:
        draw.text(position, word, font=font, fill=get_random_text_color())
    else:
        # If still doesn't fit vertically, skip the word
        pass
...
def apply_random_blur(draw, image, word, font, position):
    ...
    if blur:
        ...
        blurred_part = temp_image.filter(ImageFilter.GaussianBlur(random.uniform(1, 5)))  # More blur
...
def overlay_paper_with_rotated_text(background_img, paper_img, corpus_file, font_path, output_path):
    ...
    paper = paper.filter(ImageFilter.GaussianBlur(random.uniform(1, 3)))
    ...
    for row in range(grid_rows):
        for col in range(grid_cols):
            ...
            word_x = col * cell_width
            word_y = row * cell_height

            # Draw the word within the cell bounds, adjusting the font size if necessary
            draw_text_within_bounds(draw_on_paper, word, font, cell_width, cell_height, (word_x, word_y))
            word_index += 1
